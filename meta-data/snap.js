(function(d) {
  let p = d.body;  // container of slides; assume for now
  ...
  function findContainer(s, n = 1) {
    ...
    // if body doesn't contain headings or s, look into children
  ...
  if (!findContainer(s1, 3)) {
    // if not enough s found in children; look for instead
    if (p.tagName === 'BODY') {
  ...
  while (true) {
    ...
    if (!el) break;
    // remove slide separators () and create new slide
    if (isSep(el)) {
      s = newSlide(s);
  ...
  // Removed: sty.innerHTML = `:root{--slide-ratio:${screen.height / screen.width}}`;
  ...
  function setScale(e) {
    ...
    // Removed: let h = window.innerHeight, p = h / 900, r2 = h / window.innerWidth, full = d.fullscreenElement;
    // Removed: dC.toggle('slide-mode', full || r2 <= ratio) && (!full && (dE.scrollWidth > dE.offsetWidth)) && dC.remove('slide-mode');
    // Removed: sty.innerHTML = `:root{--slide-ratio:${ratio};--slide-scale:${p};--slide-top:${(p - 1)/2 * d.body.scrollHeight + 'px'};}`;
