...
6. Make a test decision about the null hypothesis $H_0$. We reject $H_0$ if the $P$-value less than the significance level $\alpha$.

7. State an overall conclusion.

---

## Example: Test a Mean with Known $\sigma$ (1 of 2)
...
Since the population standard deviation is $\sigma=4$. We use the normal distribution to find the $P$-value.

---

## Example: Test a Mean with Known $\sigma$ (2 of 2)
...
## Example: Test a Mean with Unknown $\sigma$ (1 of 2)
...
## Example: Test a Mean with Unknown $\sigma$ (2 of 2)

The $t$-test statistics is $t=\frac{18-20}{2.7/\sqrt{10}}\approx`r round((18-20)/2.7*sqrt(10), 3)`.$ In Excel, the $P$-value can be calculated as \\(p\approx\\)`=2*T.DIST(-2.342,9,TRUE)`=`r round(2*pt(round((18-20)/2.7*sqrt(10), 3),9),4)`.
...
## Example: Test a Mean from a Data Set (1 of 2)
...
## Example: Test a Mean from a Data Set (2 of 2)
<!--# style="font-size: 0.95em;"-->
...
The degree of freedom is $\text{df}=40-1$ The $P$-value is the right tail area under the $t$-curve, that is `=1-T.DIST(2.013, 39, TRUE)`=`r round(1-pt(2.013, 39),4)`.
...
$$\textstyle z=\frac{\hat{p}-p_0}{\sqrt{p_0(1-p_0)/n}}=\frac{0.32-0.5}{\sqrt{0.5(1-0.5)/50}}\approx `r round((0.32-0.5)/sqrt(0.5*0.5/50),2)`.$$

Because $H_a$ contains the $<$ sign, the test is left-tailed and the $P$-value is then `=NORM.S.DIST(-2.55,TRUE)`\\(\approx 0.005\\).

Because $0.005<0.01=\alpha$, we reject the null hypothesis $H_0$. Therefore, at the significance level 0.01, there is enough evidence to claim that the coin favors the tail.

---

## Example: Proportion of Newborns (1 of 2)
...
## Example: Proportion of Newborns (2 of 2)
...
From $H_a$, we know that the test is two-tailed. The $P$-value is then
$$2\cdot(1-P(Z<`rnewbornstat`))\approx`r 2*(1-round(pnorm(newbornstat, 0,1),3))`$$
...
## A Remark on the SE for Sample Proportion
...
## Practice: Testing the Mean GPA with known $\sigma$ {.unnumbered}

`r knitr::include_url('https://www.myopenmath.com/embedq2.php?id=941399&seed=2020&showansafter', height='500px')`

---

## Practice: Testing the Mean GPA with Unknown $\sigma$ {.unnumbered}

`r knitr::include_url('https://www.myopenmath.com/embedq2.php?id=941405&seed=2020&showansafter', height='500px')`

---

## Practice: Testing the Proportion of People Who Own Cats {.unnumbered}

`r knitr::include_url('https://www.myopenmath.com/embedq2.php?id=1231702&seed=2020&showansafter', height='500px')`

---

## Practice: Testing the Mean Recovery Time {.unnumbered}
...
::: {.footmark}
Source: [Exercise 15 in Section 8.4 in Introductory Statistics](https://saylordotorg.github.io/text_introductory-statistics/s12-04-small-sample-tests-for-a-popul.html).
:::
...
## Lab Practice: A Car Manufacturer's Claim {.unnumbered}
