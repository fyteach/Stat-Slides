...
- In Excel, $z_{\alpha/2}$=`NORM.S.INV((1+confidence level)/2)` and  
  $E=z_{\alpha/2}\frac{\sigma}{\sqrt{n}} =$ `CONFIDENCE.NORM(1-confidence level, sigma, n)`.
...
Applying the Excel functions `AVERAGE()` and `STDEV.S()` to the data, we find that $\bar{x}\approx 29.6$ and $s\approx 5.3$.
