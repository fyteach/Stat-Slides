...
Suppose the population standard deviation $\sigma$ is given.
...
- Find the standard error $\text{SE}=\sigma_{\bar{x}}=\sigma/\sqrt{n}=$ `=(Population SD)/SQRT(Sample Size)`.
...
1. Using Excel function NORM.S.INV(0.5+0.9/2), we get the critical value
   $$z_{\alpha/2}=`r round(qnorm((1-0.1/2), 0,1),4)`.$$

2. Using the Excel function `NORM.S.INV(0.5+0.98/2)`, we get the critical value
...
Suppose the population standard deviation is NOT given.
...
- Find the estimated standard error $\text{SE}=\sigma_{\bar{x}}=s/\sqrt{n}=$ `=(Sample SD)/SQRT(Sample Size)`.
